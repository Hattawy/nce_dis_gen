	subroutine radaftB12(I, Q2, Ein, Eout, depol)
...
c       SK: The following parameters can be gotten from the Excel spreadsheet PreRad_eloss.xls
	REAL*4 PKmin	! Probability of loss>Kmin
	REAL*4 const1, const2 /0.06501/	! radiation loss parameters, product = approx. bt
    	REAL*4 aoffset, aslope, pkminoffset, pkminslope
    	REAL*4 aso/14.0953/,ass/-0.2732/,aoo/0.0744/,aos/0.0078/
    	REAL*4 pso/6.2298/,pss/0.6363/,poo/0.0289/,pos/0.0090/
	INTEGER I
	
c    	write(6,1) Ein, Q2
1   	format(' AFTER:  Ein: ',F9.5,' Q2: ',F9.5)
   	Eout = Ein
    	if(Ein.lt.1.0) return
    	aoffset = aoo + aos*alog(Ein)
    	aslope = aso + ass*alog(Ein)
    	pkminoffset = poo + pos*alog(Ein)
    	pkminslope = pso + pss*alog(Ein)
